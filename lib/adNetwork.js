...
// Liste toutes les apps média actives (triées par display_order)
export async function getMediaApps(supa) {
  const { data } = await supa
    .from('media_apps')
    .select('id, code, name_vi, emoji, color, display_order')
    .eq('active', true)
    .order('display_order', { ascending: true });
  return data || [];
}

// Résout un code d'app (ex: 'van-menh') → la ligne complète media_apps
// Retourne null si le code n'existe pas ou que l'app est désactivée
export async function getMediaAppByCode(supa, code) {
  if (!code) return null;
  const { data } = await supa
    .from('media_apps')
    .select('id, code, name_vi, emoji, color, active')
    .eq('code', code)
    .maybeSingle();
...
// Récupère la ligne media_apps à partir d'un id (utile pour enrichir une campagne)
export async function getMediaAppById(supa, id) {
  if (!id) return null;
  const { data } = await supa
    .from('media_apps')
    .select('id, code, name_vi, emoji, color')
    .eq('id', id)
    .maybeSingle();
  return data || null;
}
